...
server lightweight, which is great for microservices. It also deploys to every major cloud platform,
...
feature in the [hotspot=1-20]`server.xml`.
[role="code_command hotspot=7-7", subs="quotes"]
----
...
Take a look at your [hotspot=1-269]`pom.xml` file.

The loose application support is enabled with the [hotspot=196]`<looseApplication/>`
element in the [hotspot=183-205]`liberty-maven-plugin` plug-in.
...
microservices that are running inside of the server. 


[role="code_command hotspot=12-36", subs="quotes"]
----
Create the `SystemHealth` class.
`src/main/java/io/openliberty/sample/system/SystemHealth.java`
----

The [hotspot=12-36]`SystemHealth` class allows you to report on the health status of the `system` microservice.
...
These messages are also logged to the `target/liberty/wlp/usr/servers/GettingStartedServer/logs/console.log`
...
[role="code_command hotspot=14" subs="quotes"]
----
Edit the server configuration file.
`src/main/liberty/config/server.xml`
----

By adding the [hotspot=14]`<logging/>` element you are enabling detailed logging of the MicroProfile Health feature.
...
a `usr` package that contains the servers and application to be extracted onto an Open Liberty
installation. 


The type of server package is configured with [hotspot=33 file=0]`<packaging.type/>` in the 
[hotspot=1-269 file=0]`pom.xml`.
...
and server at the same time. To generate a runnable JAR, invoke the [hotspot=40-45 file=0]`runnable-package` profile by using the `-P` flag:
[role='command']
```
...
[hotspot=40-45 file=0]`runnable-package` profile is invoked, which temporarily overrides the [hotspot=43 file=0]`packaging.type` property from the `usr` package to the [hotspot=43 file=0]`runnable` package. This property then propagates to the [hotspot=183-205 file=0]`liberty-maven-plugin` plug-in, which generates the server package that you want.
...
the entirety of Java EE and MicroProfile. As a result, this JAR is over 100 MB. To omit
the features that you don't need and package the JAR with only the features that you defined in the 
[hotspot=1-20 file=1]`server.xml` file, use `minifiy,runnable` as the packaging type. To build a minimal runnable JAR, invoke the [hotspot=46-51 file=0]`minify-runnable-package` profile by using the `-P` flag:
[role='command']
```
mvn install -P minify-runnable-package
```

The [hotspot=46-51 file=0]`minify-runnable-package` profile overrides the [hotspot=43 file=0]`package.type` property from the `usr` package to the `minify,runnable` package and generates a runnable JAR file that contains only the features that you explicitly enabled in your 
[hotspot=1-20 file=1]`server.xml` file. As a result, the generated JAR is only about 37 MB.
...
To run the application in a container, you need to have Docker installed. For installation instructions, 
see the https://docs.docker.com/install/[Official Docker Docs].
...
The [hotspot=52-75]`docker-image` profile uses the [hotspot=58]`dockerfile-maven` plug-in, which automatically builds a Docker image from a `Dockerfile` that is located in the same directory as the [hotspot=1-269]`pom.xml` file.
...
To build and containerize the application, start your Docker daemon and then invoke the profile
by using the `-P` flag:
[role='command']
```
