...
When enabled, the [hotspot=7 file=0]`mpHealth` feature automatically adds a `/health` endpoint to the application.
...
The [hotspot=12-36 file=0]`SystemHealth` class allows you to report on the health status of the `system` microservice.
...
include::finish/src/main/liberty/config/server.xml[tags=**]
----
By adding the [hotspot=14 file=0]`<logging/>` element you are enabling detailed logging of the MicroProfile Health feature.
...
The type of server package is configured with [hotspot=33]`<packaging.type/>` in the 
[hotspot=1-269]`pom.xml`.
...
and server at the same time. To generate a runnable JAR, invoke the [hotspot=40-45]`runnable-package` profile by using the `-P` flag:
[role='command']
```
