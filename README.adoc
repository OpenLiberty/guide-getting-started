...
feature in the [hotspot=1-19]`server.xml`.
...
Take a look at your [hotspot=1-288]`pom.xml` file.
...
The [hotspot=12-36]`SystemHealth` class allows you to report on the health status of the `system` 
microservice.
...
include::finish/src/main/liberty/config/server.xml[tags=**]
----
By adding the [hotspot=14]`<logging/>` element you are enabling detailed logging of the MicroProfile
Health feature.
...
The type of server package is configured with [hotspot=33]`<packaging.type/>` in the `pom.xml`.
...
The `-P` flag specifies the Maven profile to be run during the build. In this case, the [hotspot=40-45]`runnable-package`
profile is invoked, which temporarily overrides the `packaging.type` property from the `usr` package 
to the [hotspot=43]`runnable` package. This property then propagates to the `liberty-maven-plugin` plug-in, which 
...
The [hotspot=46-51]`minify-runnable-package` profile overrides the `package.type` property from the `usr` package to the 
`minify,runnable` package and generates a runnable JAR file that contains only the features that you 
explicitly enabled in your `server.xml` file. As a result, the generated JAR is only about 37 MB.
...
The [hotspot=52-75]`docker-image` profile uses the [hotspot=58]`dockerfile-maven` plug-in, which automatically builds a Docker image from a `Dockerfile` that is located in the same directory as the `pom.xml` file.
