...
Later, when you need to stop the server, simply type `q` in the shell session where
...
Afterwards, stop the server by typing `q` in the shell session where you ran the server and then press the `enter/return` key.
...
At this point, you can stop the server by typing `q` in the shell session that
the server runs in and then press the `enter/return` key.
