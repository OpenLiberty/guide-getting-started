...
You will learn how to run a simple REST microservice on an Open Liberty server. You will use Maven to
build and run this microservice, as well as to control the Open Liberty server.
...
To begin, build the simple REST microservice we've provided and run it in Open Liberty. To do this,
...
The `mvn` command runs a Maven build, during which the `target` directory is created to store all
build-related files.
...
and the application is installed into that server via loose config.
...
Let's try updating the server configuration while the server is running. The `system` microservice does
...
If your IDE is configured to recompile each time you save your changes, you don't even need to manually
recompile through Maven.
...
reports that the server is running but it doesn't provide any details of the microservice that runs
inside the server. To report on the health status of the microservice, create a new file `src/main/java/io/openliberty/sample/system/SystemHealth.java`:
...
Try enabling detailed logging of the `mpHealth` feature you added earlier. To do this, add the following
...
Feel free to stop the server at this point by hitting `CTRL+C`.
