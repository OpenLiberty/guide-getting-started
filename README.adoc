...
by running the Maven `package` phase and the Maven `liberty:run-server` goal from the
`start` directory:
...
Besides the `run-server` goal, the Open Liberty Maven plugin also includes a new `dev` goal, which listens for any changes in the project, whether it's application source code or configuration.

If your server is still running, stop it again by pressing `CTRL+C` in the shell session where you ran the server, or by running the `liberty:stop-server` goal from the `start` directory in another shell session.
...
Similarily, access the `/health/live` endpoint by visiting the http://localhost:9080/health/live[http://localhost:9080/health/live^] URL to view the data from the liveness health check.
...
A similar goal can be achieved by running the `run-server` goal and manually re-packaging or re-compiling the application using `mvn package` or `mvn compile`, respectively, while the server is running.
