...
is configured to include the `liberty-maven-plugin` Liberty Maven plug-in, which allows 
...
To access the `system` microservice, visit the http://localhost:9080/system/properties URL, 
...
http://localhost:9080/health/ URL, you see a 404 error because the `/health` endpoint 
does not yet exist:
...
Try to access the `/health` endpoint again by visiting the http://localhost:9080/health 
...
The [hotspot=12-36]`SystemHealth` class allows you to report on the health status of the 
`system` microservice.
...
Access the `/health` endpoint again by visiting the http://localhost:9080/health URL. 
...
the [hotspot=1-269 file=0]`pom.xml`.
...
[hotspot=40-46 file=0]`runnable-package` profile by using the `-P` flag:
...
[hotspot=40-45 file=0]`runnable-package` profile is invoked, which temporarily overrides 
the [hotspot=43 file=0]`packaging.type` property from the `usr` package to the `runnable` 
package. This property then propagates to the [hotspot=183-205 file=0]`liberty-maven-plugin` 
...
the features that you defined in the [hotspot=1-20 file=1]`server.xml` file, use 
`minifiy,runnable` as the packaging type. To build a minimal runnable JAR, invoke the 
[hotspot=46-51 file=0]`minify-runnable-package` profile by using the `-P` flag:
...
The [hotspot=46-51 file=0]`minify-runnable-package` profile overrides the 
[hotspot=49 file=0]`package.type` property from the `usr` package to the `minify,runnable` 
package and generates a runnable JAR file that contains only the features that you 
explicitly enabled in your [hotspot=1-20 file=1]`server.xml` file. As a result, the 
generated JAR is only about 37 MB.
...
When the server starts, visit the http://localhost:9080/system/properties URL to access 
...
The [hotspot=52-75]`docker-image` profile uses the [hotspot=58]`dockerfile-maven` plug-in, 
which automatically builds a Docker image from a `Dockerfile` that is located in the same 
directory as the [hotspot=1-269]`pom.xml` file.
