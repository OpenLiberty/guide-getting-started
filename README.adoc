...
The type of server package is configured with [hotspot=35 file=0]`<packaging.type/>` in the 
...
and server at the same time. To generate a runnable JAR, invoke the [hotspot=42-47 file=0]`runnable-package` profile by using the `-P` flag:
[role='command']
```
...
[hotspot=42-47 file=0]`runnable-package` profile is invoked, which temporarily overrides the [hotspot=45 file=0]`packaging.type` property from the `usr` package to the [hotspot=45 file=0]`runnable` package. This property then propagates to the [hotspot=185-207 file=0]`liberty-maven-plugin` plug-in, which generates the server package that you want.
...
[hotspot=1-20 file=1]`server.xml` file, use `minifiy,runnable` as the packaging type. To build a minimal runnable JAR, invoke the [hotspot=48-53 file=0]`minify-runnable-package` profile by using the `-P` flag:
[role='command']
```
mvn install -P minify-runnable-package
```

The [hotspot=48-53 file=0]`minify-runnable-package` profile overrides the [hotspot=45 file=0]`package.type` property from the `usr` package to the `minify,runnable` package and generates a runnable JAR file that contains only the features that you explicitly enabled in your 
