...
by running the Maven `package` phase and the Maven `liberty:dev` goal from the
`start` directory:
...
The `liberty:dev` argument specifies the Open Liberty `dev` goal, which
...
`target` directory, that is you need to invoke Maven `package`.
