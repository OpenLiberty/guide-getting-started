...
your application and server at the same time. To generate a runnable JAR, override the 
`include` and `packageFile` properties: 
[role='command']
```
mvn liberty:package-server -Dinclude=runnable -DpackageFile=target/getting-started.jar
```

The packaging type will be overrided from the `usr` package to the `runnable`
...
When the build completes, you can find the minimal runnable `getting-started.jar` file in the
`target` directory. This JAR file contains only the [hotspot=2-9 file=1]`features` that you
explicitly enabled in your [hotspot file=1]`server.xml` file, 
and omits other Open Liberty features that you don't need. As a result, the
