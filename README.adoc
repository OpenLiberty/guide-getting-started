...
The Open Liberty Maven plug-in includes a `devc` goal that builds a Docker
...
Similar to the previously discussed development mode, this development mode with
a container allows you to run tests by pressing the `enter/return` key. 


When you are finished, exit development mode by pressing CTRL+C in the
command-line session that the container was started from from, or by typing `q`
