...
Take a look at your [hotspot=1-271]`pom.xml` file.

The loose application support is enabled with the [hotspot=198]`<looseApplication/>`
element in the [hotspot=185-207]`liberty-maven-plugin` plug-in.
...
and server at the same time. To generate a runnable JAR, invoke the [hotspot=40-46 file=0]`runnable-package` profile by using the `-P` flag:
[role='command']
```
...
[hotspot=40-46 file=0]`runnable-package` profile is invoked, which temporarily overrides the [hotspot=44 file=0]`packaging.type` property from the `usr` package to the `runnable` package. This property then propagates to the [hotspot=185-207 file=0]`liberty-maven-plugin` 
...
[hotspot=1-20 file=1]`server.xml` file, use `minifiy,runnable` as the packaging type. 
To build a minimal runnable JAR, invoke the [hotspot=47-53 file=0]`minify-runnable-package`
...
The [hotspot=47-53 file=0]`minify-runnable-package` profile overrides the [hotspot=51 file=0]`package.type` property from the `usr` package to the `minify,runnable` package and generates a runnable JAR file that contains only the features that you explicitly enabled in your 
...
The [hotspot=54-77]`docker-image` profile uses the [hotspot=60]`dockerfile-maven` plug-in, which automatically builds a Docker image from a `Dockerfile` that is located in the same directory as the [hotspot=1-271]`pom.xml` file.
