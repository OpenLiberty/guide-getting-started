...
If you re-package the server, including its configuration, while the server is running, Liberty will automatically reload the configuration without restarting.
...
If you're server is still running, stop it again by pressing `CTRL+C` in the shell session where you ran the server, or by running the `liberty:stop-server` goal from the `start` directory in another shell session.
...
This will start Open Liberty in the development mode, which listens for file changes and otherwise enables us to access the server in the same way as before:
...
After you've made the file changes, Open Liberty will automatically reload its configuration.
...
After you've made the file changes, Open Liberty will automatically reload its configuration and the `system` application.
