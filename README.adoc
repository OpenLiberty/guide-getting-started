...
by running the Maven `package` phase and the Maven `liberty:run` goal from the
`start` directory:
...
The `package` argument specifies the Maven `package` phase. During this phase, the
application is built and packaged into a `.war` file.
...
When you update the server configuration files, you can run the `mvn package` command to invoke the Maven `package` phase which executes various Maven goals that repackage the server.
If you re-package the server, including its configuration, while the server is running, Liberty automatically reloads the configuration without restarting.
This allows for quicker turnarounds and an improved developer experience.

Besides the `run` goal, the Open Liberty Maven plugin also includes a new `dev` goal, which listens for any changes in the project, whether it's application source code or configuration.

If your server is still running, stop it again by pressing `CTRL+C` in the shell session where you ran the server, or by running the `liberty:stop` goal from the `start` directory in another shell session.

To run the `dev` phase of the Maven plugin, invoke the Maven `package` phase and the `liberty:dev` goal in the `start` directory:
...
This starts Open Liberty in the development mode, which listens for file changes and otherwise enables us to access the server in the same way as before:

You can visit the http://localhost:9080/system/properties[http://localhost:9080/system/properties^] URL just as before.
...
To add the MicroProfile Health feature to the server, include the [hotspot=7]`mpHealth` feature in the [hotspot]`server.xml`.
...
After you've made the file changes, Open Liberty automatically reloads its configuration.
When enabled, the [hotspot=7]`mpHealth` feature automatically adds a `/health` endpoint to the application.
You can see the server being updated in the server log that's displayed in your shell session:
...
This time you'll see the following JSON:
...
At the moment, the `/health` endpoint reports whether or not the server is running, but the endpoint doesn't provide any details on the microservices that are running inside of the server.
...
----
include::finish/src/main/java/io/openliberty/sample/system/SystemReadinessCheck.java[tags=**;!copyright]
----

The [hotspot=16-32 file=1]`SystemReadinessCheck` class verifies that the `system` microservice is not in maintenance by checking a config property.
...
[source, java, linenums, role='code_column']
----
include::finish/src/main/java/io/openliberty/sample/system/SystemLivenessCheck.java[tags=**;!copyright]
----

The [hotspot=14-29 file=2]`SystemLivenessCheck` class reports a status of `DOWN` if the microservice uses over 90% of the maximum amount of memory.

After you've made the file changes, Open Liberty automatically reloads its configuration and the `system` application.
...
You can also access the `/health/ready` endpoint by visiting the http://localhost:9080/health/ready[http://localhost:9080/health/ready^] URL to view the data from the readiness health check.
Similarly, access the `/health/live` endpoint by visiting the http://localhost:9080/health/live[http://localhost:9080/health/live^] URL to view the data from the liveness health check.
...
A similar goal can be achieved by running the `run` goal and manually re-packaging or re-compiling the application using `mvn package` or `mvn compile`, respectively, while the server is running.
The development mode has been added to further improve the developer experience by minimizing the turnaround times.
...
Afterwards, stop the server by pressing `CTRL+C` in the shell session where you ran the server, or by running the `liberty:stop` goal from the `start` directory in another shell session.
...
following Docker build command in the `start` directory. The `.war` file needs to reside under the
`target` directory, that is we need to previously invoke Maven `package`.
