...
----
Replace the `src/main/liberty/config/server.xml` server configuration file.
----
server.xml
...
----
To report on the health status of the `system` 
...
The [hotspot=50-73]`docker-image` profile uses the `dockerfile-maven` plug-in, which automatically builds a Docker image from a `Dockerfile` that is located in the same directory as the `pom.xml` file.
