...
= Getting started with the Open Liberty application server (To be published)
...
[role="code_command hotspot=1-8", subs="quotes"]
----
Replace the `src/main/liberty/config/server.xml` server configuration file
that is located in the `start` directory to include the `mpHealth-1.0` feature.
----
server.xml
[source, xml, role='code_column', linenums]
----
...
[role="code_command hotspot=210-211", subs="quotes"]
----
The loose application support is enabled with the `<looseApplication>`
element in the `liberty-maven-plugin` plug-in.
----
pom.xml
[source, XML, indent=0, role="code_column", linenums]
----
...
[role="code_command hotspot=13-36", subs="quotes"]
----
...
----
SystemHealth.java
[source, java, role='code_column', linenums]
----
...
----
The type of server package is configured in your `pom.xml` file in the following line.
----
...
to the `runnable` package. 

[role="code_command hotspot=39-44" subs="quotes"]
----
This property then propagates to the `liberty-maven-plugin` plug-in, which 
...
[role="code_command hotspot=53-76", subs="quotes"]
----
