...
`minifiy,runnable` as the packaging type. To build a minimal runnable JAR, update the 
`-D` flag:

[role='command']
```
mvn liberty:package-server -Dinclude=minify,runnable -DpackageFile=target/getting-started.jar
```

This command overrides the packaging type from the `usr` package to the `minify,runnable`
