...
[hotspot=40-45 file=0]`runnable-package` profile is invoked, which temporarily overrides the [hotspot=43 file=0]`packaging.type` property from the `usr` package to the `runnable` package. This property then propagates to the [hotspot=183-205 file=0]`liberty-maven-plugin` 
...
To build a minimal runnable JAR, invoke the [hotspot=46-51 file=0]`minify-runnable-package`
...
The [hotspot=46-51 file=0]`minify-runnable-package` profile overrides the [hotspot=49 file=0]`package.type` property from the `usr` package to the `minify,runnable` package and generates a runnable JAR file that contains only the features that you explicitly enabled in your 
