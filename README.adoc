...
Take a look at your [hotspot=1-280]`pom.xml` file.

The loose application support is enabled with the [hotspot=194]`<looseApplication/>`
...
The `SystemHealth` class allows you to report on the health status of the `system` 
microservice.
...
include::finish/src/main/liberty/config/server.xml[tags=**]
----
By adding the `<logging/>` element you are enabling detailed logging of the MicroProfile
Health feature.
...
The type of server package is configured with [hotspot=31]`<packaging.type/>` in the `pom.xml`.
...
The `-P` flag specifies the Maven profile to be run during the build. In this case, the [hotspot=38-43]`runnable-package`
profile is invoked, which temporarily overrides the `packaging.type` property from the `usr` package 
to the [hotspot=41]`runnable` package. This property then propagates to the `liberty-maven-plugin` plug-in, which 
...
The [hotspot=44-49]`minify-runnable-package` profile overrides the `package.type` property from the `usr` package to the 
...
The [hotspot=50-73]`docker-image` profile uses the [hotspot=56]`dockerfile-maven` plug-in, which automatically builds a Docker image from a `Dockerfile` that is located in the same directory as the `pom.xml` file.
