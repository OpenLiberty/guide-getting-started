...
To add the MicroProfile Health feature to the server, include the `mpHealth-1.0` feature in the `server.xml`.
[role="code_command hotspot=6-7", subs="quotes"]
----
Add the MicroProfile Health feature to the `server` configuration file.
...
When enabled, the [hotspot=7-8]`mpHealth-1.0` feature automatically adds a `/health` endpoint to the application.
...
The loose application support is enabled with the [hotspot=205-206]`<looseApplication>`
...
----
To report on the health status of the `system` microservice, create the file.
...
Try enabling detailed logging of the MicroProfile Health feature by adding the [hotspot=13-14]`logging` element.
...
The type of server package is configured with [hotspot=30-31]`<packaging.type>` in the `pom.xml`.
...
to the [hotspot="41-42"]`runnable` package. This property then propagates to the `liberty-maven-plugin` plug-in, which 
...
The [hotspot=50-73]`docker-image` profile uses the [hotspot=58-59]`dockerfile-maven` plug-in, which automatically builds a Docker image from a `Dockerfile` that is located in the same directory as the `pom.xml` file.
