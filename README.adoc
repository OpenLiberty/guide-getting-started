...
whether it's application source code or configuration. The Liberty server automatically reloads the configuration without restarting. This allows for quicker turnarounds and an improved developer experience.
...
changes you make to your application and allows you to run tests by pressing `enter/return` key in the active shell. When you’re working on your application, rather than having to keep re-running Maven commands, you just hit `enter/return` to verify your change.
