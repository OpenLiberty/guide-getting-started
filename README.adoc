...
Finally, you will package the application along with the runtime configuration into a Docker image and run that image as a container.
...
Your [hotspot]`pom.xml` file is located in the `start` directory and is configured to include the [hotspot=libertyMavenPlugin]`liberty-maven-plugin`, which allows you to install applications into Open Liberty and manage the runtime instances.
...
The `liberty:run` argument specifies the Open Liberty `run` goal, which starts an Open Liberty instance in the foreground. As part of this phase, an Open Liberty runtime is downloaded and installed into the `target/liberty/wlp` directory, a runtume instance is created and configured in the `target/liberty/wlp/usr/servers/defaultServer` directory, and the application is installed into that instance via https://www.ibm.com/support/knowledgecenter/en/SSEQTP_liberty/com.ibm.websphere.wlp.doc/ae/rwlp_loose_applications.html[loose config^].

For more information about the Liberty Maven plug-in, see its https://github.com/WASdev/ci.maven[GitHub repository^].

When the runtime instance begins starting up, various messages display in your command-line session. Wait for the following message, which indicates that the instance startup is complete:
...
When you need to stop the instance, press `CTRL+C` in the command-line session where you ran the instance, or run the `liberty:stop` goal from the `start` directory in another command-line session:
...
// =================================================================================================
// Starting and stopping the Open Liberty instance in the background
// =================================================================================================

== Starting and stopping the Open Liberty instance in the background

Although you can start and stop the instance in the foreground by using the Maven `liberty:run` goal, you can also start and stop the instance in the background with the Maven `liberty:start` and `liberty:stop` goals:
...
// =================================================================================================
// Updating the runtime configuration without restarting the instance
// =================================================================================================
...
To add the MicroProfile Health feature to the instance, include the [hotspot=mpHealth]`mpHealth` feature in the [hotspot]`server.xml`.

[role="code_command hotspot", subs="quotes"]
----
#Replace the runtime configuration file.#
...
After you make the file changes, Open Liberty automatically reloads its configuration. When enabled, the [hotspot=mpHealth]`mpHealth` feature automatically adds a `/health` endpoint to the application. You can see the instance being updated in the instance log displayed in your command-line session:
...
Now you can verify whether your instance is up and running.


// =================================================================================================
// Updating the source code without restarting the instance
// =================================================================================================

== Updating the source code without restarting the instance

The RESTful application that contains your `system` microservice runs in an instance from its `.class` file and other artifacts. Open Liberty automatically monitors these artifacts, and whenever they are updated, it updates the running instance without the need for the instance to be restarted.
...
Try updating the source code while the instance is running in dev mode. At the moment, the `/health` endpoint reports whether the instance is running, but the endpoint doesn't provide any details on the microservices that are running inside of the instance.
...
Access the `/health` endpoint again by going to the http://localhost:9080/health[http://localhost:9080/health^] URL. This time you see the overall status of your instance and the aggregated data of the liveness and readiness checks for the `system` microservice:
...
Making code changes and recompiling is fast and straightforward. Open Liberty dev mode automatically picks up changes in the `.class` files and artifacts, without needing to be restarted. Alternatively, you can run the `run` goal and manually repackage or recompile the application by using the `mvn package` command or the `mvn compile` command while the instance is running. Dev mode was added to further improve the developer experience by minimizing turnaround times.


// =================================================================================================
// Checking the Open Liberty instance logs
// =================================================================================================

== Checking the Open Liberty instance logs

While the instance is running in the foreground, it displays various console messages in the command-line session. These messages are also logged to the `target/liberty/wlp/usr/servers/defaultServer/logs/console.log` file. You can find the complete instance logs in the `target/liberty/wlp/usr/servers/defaultServer/logs` directory. The `console.log` and `messages.log` files are the primary log files that contain console output of the running application and the Liberty instance. More logs are created when runtime errors occur or whenever tracing is enabled. You can find the error logs in the `ffdc` directory and the tracing logs in the `trace.log` file.
...
So far, Open Liberty was running out of the `target/liberty/wlp` directory, which effectively contains an Open Liberty instance and the deployed application. The final product of the Maven build is a server package for use in a continuous integration pipeline and, ultimately, a production deployment.

Open Liberty supports a number of different server packages. The sample application currently generates a `usr` package that contains the runtime and application to be extracted onto an Open Liberty installation.

Instead of creating a server package, you can generate a runnable JAR file that contains the application along with a runtime. This JAR file can then be run anywhere and deploy your application and runtime at the same time. To generate a runnable JAR file, override the  `include` property: 
[role='command']
```
...
To run the JAR file, first stop the instance if it's running. Then, navigate to the `target` directory and run the `java -jar` command:
...
When the instance starts, go to the http://localhost:9080/dev/system/properties[http://localhost:9080/dev/system/properties^] URL to access your application that is now running out of the minimal runnable JAR file.

You can stop the instance by pressing `CTRL+C` in the command-line session that the instance runs in.
...
You've learned the basics of deploying and updating an application on an Open Liberty instance.
