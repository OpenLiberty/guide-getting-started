...
Update the [hotspot file=0]`mpData.js` file to change the [hotspot=jsUrl file=0]`url` in the [hotspot file=0]`getSystemPropertiesRequest` method to reflect the new context root.
...
Update the [hotspot file=1]`server.xml` file to change the context root from `/` to `/dev`.
