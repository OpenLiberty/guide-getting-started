...
The restfulWS application that contains your `system` microservice runs in a server from its `.class` file and other artifacts. Open Liberty automatically monitors these artifacts, and whenever they are updated, it updates the running server without the need for the server to be restarted.
...
After you make the file changes, Open Liberty automatically reloads its configuration. You can access the application at the http://localhost:9080/dev/system/properties[http://localhost:9080/dev/system/properties^] URL. Notice that context root is now `/dev`.
