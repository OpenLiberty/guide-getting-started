...
Access the `/health` endpoint again by goining to the http://localhost:9080/health[http://localhost:9080/health^] URL.
...
following Docker build command in the `start` directory. Run the `mvn package` command from 
the `target` directory, which is where the `.war` file needs to reside.
