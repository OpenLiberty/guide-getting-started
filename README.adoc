...
the application inside of a container. This development mode, known as dev mode, also listens for
any changes in the application source code or configuration and rebuilds the
image and restarts the container as necessary.
...
then pressing the `enter/return` key. This stops and removes the container for you. You
can check that the container was stopped by running the `docker ps` command.
